...
function casinotheme_setup() {
    ...
    add_image_size( 'sk-small-8', 210, 140, true ); //Latest Posts_sk thumb
...
/*	register_sidebar( array(
		...
		'before_widget' => '<aside id="cpa_%1$s" class="cpa_widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="cpa_widget-title">',
		'after_title' => '</h3>',
	) );
    ...
    register_sidebar(array(
        ...
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h1 class="cpa_br4">',
        'after_title' => '</h1>',
        'before_text' => '<div class="cpa_cat_desc roz_1">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h2 class="cpa_br3">',
        'after_title' => '</h2>',
        'before_text' => '<div class="cpa_cat_desc roz_2">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h2 class="cpa_br32">',
        'after_title' => '</h2>',
        'before_text' => '<div class="cpa_cat_desc roz_3">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_title' => '<h2 class="cpa_br3">',
        'after_title' => '</h2>',
        'before_text' => '<div class="cpa_cat_desc roz_4">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_title' => '<h1 class="cpa_br4">',
        'after_title' => '</h1>',
        'before_text' => '<div class="cpa_cat_desc roz_5">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_title' => '<h2 class="cpa_br3">',
        'after_title' => '</h2>',
        'before_text' => '<div class="cpa_cat_desc roz_6">',
        'after_text' => '</div>',
    ));    
    ...
    register_sidebar(array(
        ...
        'before_title' => '<h1 class="cpa_br4">',
        'after_title' => '</h1>',
        'before_text' => '<div class="cpa_cat_desc roz_7">',
        'after_text' => '</div>',
    ));
    ...
    register_sidebar(array(
        ...
        'before_title' => '<h2 class="cpa_br3">',
        'after_title' => '</h2>',
        'before_text' => '<div class="cpa_cat_desc roz_8">',
        'after_text' => '</div>',
    ));  
...
function casinotheme_content_nav( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="cpa_<?php echo esc_attr( $html_id ); ?>" class="cpa_navigation" role="navigation">
			<h3 class="cpa_assistive-text"><?php _e( 'Post navigation', 'casinotheme' ); ?></h3>
			<div class="cpa_nav-previous"><?php next_posts_link( __( '<span class="cpa_meta-nav">&larr;</span> Older posts', 'casinotheme' ) ); ?></div>
			<div class="cpa_nav-next"><?php previous_posts_link( __( 'Newer posts <span class="cpa_meta-nav">&rarr;</span>', 'casinotheme' ) ); ?></div>
		</nav><!-- .navigation -->
	<?php endif;
}
endif;
...
function casinotheme_comment( $comment, $args, $depth ) {
	...
	?>
	<li <?php comment_class(); ?> id="cpa_comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'casinotheme' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'casinotheme' ), '<span class="cpa_edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
		...
		global $post;
	?>
	<li <?php comment_class(); ?> id="cpa_li-comment-<?php comment_ID(); ?>">
		<article id="cpa_comment-<?php comment_ID(); ?>" class="cpa_comment">
			<header class="cpa_comment-meta comment-author vcard">
				<?php
					echo get_avatar( $comment, 44 );
					printf( '<cite><b class="cpa_fn">%1$s</b> %2$s</cite>',
			...
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="cpa_comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'casinotheme' ); ?></p>
			<?php endif; ?>

			<section class="cpa_comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Edit', 'casinotheme' ), '<p class="cpa_edit-link">', '</p>' ); ?>
...
function casinotheme_entry_meta() {
	...
	$date = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><time class="cpa_entry-date" datetime="%3$s">%4$s</time></a>',
	...
	$author = sprintf( '<span class="cpa_author vcard"><a class="cpa_url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
	...
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="cpa_by-author"> by %4$s</span>.', 'casinotheme' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="cpa_by-author"> by %4$s</span>.', 'casinotheme' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="cpa_by-author"> by %4$s</span>.', 'casinotheme' );
	}
...
public function widget( $args, $instance ) {
    ...
    if ( ! empty( $title ) )
        //echo $args['before_title'] . $title . $args['after_title'];
      		...
      		echo '  <div class="cpa_top_banner_one">
                        <a href="'.$title4.'" target="_blank" class="cpa_top_banner_img" title="'.$title.'"><img src="'.$title2.'" alt="'.$title.'"/></a>
                    </div>';
...
?>
    <p>
        ...
        <input class="cpa_widefat" id="cpa_<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
    </p>
    
    <p>
        <label for="<?php echo $this->get_field_id( 'title2' ); ?>">Картинка</label> 
        <input class="cpa_widefat" id="cpa_<?php echo $this->get_field_id( 'title2' ); ?>" name="<?php echo $this->get_field_name( 'title2' ); ?>" type="text" value="<?php echo esc_attr( $title2 ); ?>" />
    </p>
    
    <p>
        <label for="<?php echo $this->get_field_id( 'title4' ); ?>">Ссылка url</label> 
        <input class="cpa_widefat" id="cpa_<?php echo $this->get_field_id( 'title4' ); ?>" name="<?php echo $this->get_field_name( 'title4' ); ?>" type="text" value="<?php echo esc_attr( $title4 ); ?>" />
    </p>
...
global $wp_query, $wp_rewrite;
  ...
  if ($max > 1) echo '<div class="cpa_page_all">';
  if ($total == 1 && $max > 1) $pages = '<div class="cpa_page_name">Страницы:</div>'."\r\n";
